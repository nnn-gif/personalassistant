...
impl AudioTranscriber {
    ...
    async fn transcribe_with_whisper(&self, audio_path: &Path) -> Result<TranscriptionResult> {
        ...
        match self.transcribe_with_available_methods(audio_path).await {
            Ok(transcription_text) => {
                ...
                let enhanced_text = self.enhance_transcription_with_ollama(&transcription_text).await?;
                let segments = self.create_segments_from_text(&enhanced_text, audio_info.duration_seconds);
            ...
            Err(e) => {
                println!("Available transcription methods failed: {}, falling back to Ollama", e);
            }
        }
    ...
    async fn transcribe_with_available_methods(&self, audio_path: &Path) -> Result<String> {
        ...
        Err(AppError::Audio("No transcription method produced results".into()))
    }
    ...
    async fn analyze_audio_file(&self, audio_path: &Path) -> Result<String> {
        ...
        let speech_samples = samples.iter().filter(|&&s| safe_abs(s) > silence_threshold).count();
    ...
    async fn transcribe_with_ollama(&self, audio_path: &Path) -> Result<String> {
        ...
        #[cfg(feature = "vosk")]
        {
            if let Ok(vosk_result) = self.transcribe_with_vosk(audio_path) {
                if !vosk_result.text.trim().is_empty() {
                    // Enhance with Ollama
                    return self.enhance_transcription_with_ollama(&vosk_result.text).await;
                }
            }
        }
        ...
        if let Ok(transcription_result) = self.transcribe_with_available_methods(audio_path).await {
            // Enhance with Ollama
            return self.enhance_transcription_with_ollama(&transcription_result).await;
        }
        ...
        Err(AppError::Audio(
            ...
            with Ollama enhancement for transcription generation.".to_string()
        ))
    }
    ...
    async fn transcribe_with_system_speech(&self, _audio_path: &Path) -> Result<String> {
        ...
        Err(AppError::Audio("System speech recognition not available".into()))
    }
    ...
    async fn transcribe_with_web_speech(&self, _audio_path: &Path) -> Result<String> {
        ...
        Err(AppError::Audio("Web speech recognition not implemented".into()))
    }
    ...
    fn create_segments_from_text(&self, text: &str, total_duration: f64) -> Vec<TranscriptionSegment> {
    ...
    fn transcribe_with_vosk(&self, audio_path: &Path) -> Result<VoskResult> {
        ...
        let samples: Vec<i16> = if spec.sample_rate == 16000 && spec.channels == 1 {
            ...
            let samples: Vec<i16> = reader.samples::<i16>().collect::<std::result::Result<Vec<_>, _>>()
                .map_err(|e| AppError::Audio(format!("Failed to read samples: {}", e)))?;
    ...
    #[cfg(feature = "vosk")]
    fn resample_audio(&self, samples: &[i16], original_rate: u32, channels: u16) -> Result<Vec<i16>> {
    ...
    async fn enhance_transcription_with_ollama(&self, raw_text: &str) -> Result<String> {
        ...
        self.llm_client.send_request(&prompt).await
