...
impl AudioProcessor {
    ...
    pub fn split_audio(
        ...
        let samples_per_segment =
            (spec.sample_rate as f32 * segment_duration_seconds) as usize * spec.channels as usize;
        ...
        for chunk in samples.chunks(samples_per_segment) {
            let segment_path = base_path.join(format!(
                "{}_segment_{}.wav",
    ...
    pub fn reduce_noise(&self, input_path: &Path, output_path: &Path) -> Result<()> {
        ...
        let samples =
            samples.map_err(|e| AppError::Audio(format!("Failed to read samples: {}", e)))?;
    ...
    fn resample(&self, input: &[f32], from_rate: u32, to_rate: u32) -> Result<Vec<f32>> {
        ...
        resampler
            .process_into_buffer(&input_frames, &mut output, None)
