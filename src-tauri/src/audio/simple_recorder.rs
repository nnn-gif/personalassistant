...
impl SimpleAudioRecorder {
    ...
    pub fn stop_recording(&self) -> Result<AudioRecording> {
        ...
        let active_recording = current_guard.take()
        ...
        *active_recording.ended_at.lock().unwrap() = Some(ended_at);
        ...
        let duration = sample_count as f64 / (active_recording.sample_rate as f64 * active_recording.channels as f64);
        ...
        let recording = AudioRecording {
            ...
            title: format!("Recording {}", active_recording.info.started_at.format("%Y-%m-%d %H:%M:%S")),
            started_at: active_recording.info.started_at.to_rfc3339(),
            ended_at: ended_at.to_rfc3339(),
            duration_seconds: duration,
            file_path: active_recording.info.file_path.to_string_lossy().to_string(),
            file_size_bytes: metadata.len(),
