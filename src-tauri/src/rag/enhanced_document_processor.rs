...
impl EnhancedDocumentProcessor {
    pub fn new() -> Self {
        Self {
            ...
            supported_extensions: vec![
                // Text files
                "txt", "md", "markdown", "rst", "asciidoc", "org",
                // Programming languages
                "rs", "py", "js", "ts", "go", "java", "cpp", "c", "h", "hpp",
                "cs", "php", "rb", "swift", "kt", "scala", "clj", "hs", "ml",
                "r", "jl", "dart", "lua", "perl", "sh", "bash", "zsh", "fish",
                // Web technologies
                "html", "htm", "xml", "css", "scss", "sass", "less", "jsx", "tsx", "vue",
                // Configuration files
    ...
    pub async fn process_file(&self, file_path: &str) -> Result<ProcessedDocument> {
        ...
        println!("✅ Metadata extracted - Size: {} bytes, Type: {}", metadata.file_size, metadata.file_type);
        ...
        println!("✅ Content extracted - Raw length: {} characters", content.len());
        
        // Clean content
        println!("🧹 Cleaning content...");
        let cleaned_content = self.clean_content(&content);
        println!("✅ Content cleaned - Final length: {} characters", cleaned_content.len());
        ...
        doc_metadata.insert("processing_time_ms".to_string(), processing_time.to_string());
        ...
        println!("   - Language: {}", processed_doc.language.as_deref().unwrap_or("unknown"));
    ...
    fn validate_file(&self, path: &Path) -> Result<()> {
        ...
        if !path.is_file() {
            return Err(AppError::InvalidInput(format!("Path is not a file: {}", path.display())));
        }
    ...
    fn extract_file_metadata(&self, path: &Path) -> Result<DocumentMetadata> {
        ...
        let modified_time = fs_metadata.modified()
            .ok()
            .and_then(|t| t.duration_since(std::time::UNIX_EPOCH).ok())
            .and_then(|d| DateTime::from_timestamp(d.as_secs() as i64, 0));

        let created_time = fs_metadata.created()
            .ok()
            .and_then(|t| t.duration_since(std::time::UNIX_EPOCH).ok())
            .and_then(|d| DateTime::from_timestamp(d.as_secs() as i64, 0));
        ...
        match mime_type.type_() {
            ...
            mime::APPLICATION => {
                match mime_type.subtype().as_str() {
                    ...
                    _ => {
                        println!("❓ Unknown application type ({}), attempting text processing", mime_type.subtype());
            ...
            _ => {
                println!("❓ Unsupported MIME type ({}), attempting text processing", mime_type);
                // Try to read as text for other types
                self.process_text_file(path).await.or_else(|_| {
                    println!("❌ Text processing failed, treating as unsupported file");
                    Ok(format!("Unsupported file type ({}): {}", 
                        mime_type, 
                        path.file_name().unwrap_or_default().to_string_lossy()))
                })
            }
    ...
    async fn process_text_file(&self, path: &Path) -> Result<String> {
        ...
        if content.len() > self.max_content_length {
            println!("⚠️  Content length exceeds maximum: {} > {}", content.len(), self.max_content_length);
            return Err(AppError::ProcessingError(format!(
    ...
    async fn process_pdf_file(&self, path: &Path) -> Result<String> {
        ...
        for (page_num, _) in pages.iter().enumerate() {
            ...
            match doc.extract_text(&[(page_num + 1) as u32]) {
                Ok(page_text) => {
                    println!("✅ Page {} extracted - {} characters", page_num + 1, page_text.len());
                    
                    // Check if extraction is mostly question marks (indicates encoding issues)
                    let question_mark_ratio = if page_text.len() > 0 {
                        page_text.chars().filter(|&c| c == '?').count() as f32 / page_text.len() as f32
                    } else {
                        0.0
                    };
                    
                    // Show actual extracted content for debugging
                    println!("📄 Raw text sample (first 200 chars): {}", page_text.chars().take(200).collect::<String>());
                    
                    // Check for specific lopdf encoding errors
                    if page_text.contains("Identity-H Unimplemented") || page_text.contains("Identity-V Unimplemented") {
                        ...
                        match self.extract_pdf_with_fallback(path.to_str().unwrap()).await {
                            Ok(fallback_text) => {
                                if !fallback_text.trim().is_empty() {
                                    println!("✅ pdf-extract fallback successful - {} characters", fallback_text.len());
                                    text.push_str(&fallback_text);
                                    text.push('\n');
                                } else {
                                    println!("❌ pdf-extract fallback also failed - no readable text");
                                }
                            }
                            Err(e) => {
                    ...
                    if question_mark_ratio > 0.5 {
                        println!("⚠️  PDF page {} has {:.1}% question marks - likely encoding/OCR issue", page_num + 1, question_mark_ratio * 100.0);
                    ...
                    if !cleaned_page.trim().is_empty() {
                        ...
                        text.push('\n');
                        println!("✅ Page {} cleaned - {} characters", page_num + 1, cleaned_page.len());
                    } else {
                        println!("⚠️  Page {} contains no readable text after cleaning", page_num + 1);
                    }
                }
                Err(e) => {
                    println!("❌ Warning: Could not extract text from page {}: {}", page_num + 1, e);
                }
            }
        }
        ...
        if text.trim().is_empty() {
            ...
            return Err(AppError::ProcessingError("No readable text found in PDF".to_string()));
        }

        println!("✅ PDF processing completed - Total text: {} characters", text.len());
        Ok(text)
    }
    ...
    fn clean_pdf_text(&self, text: &str) -> String {
        ...
        cleaned = Regex::new(r"\b[a-zA-Z]\b").unwrap().replace_all(&cleaned, "").to_string();
        
        // Remove lines with only numbers (likely page numbers)
        cleaned = Regex::new(r"(?m)^\s*\d+\s*$").unwrap().replace_all(&cleaned, "").to_string();
    ...
    fn clean_content(&self, content: &str) -> String {
        ...
        println!("🔧 Applying {} cleanup patterns...", self.cleanup_patterns.len());
        for (i, pattern) in self.cleanup_patterns.iter().enumerate() {
            let before_len = cleaned.len();
            cleaned = pattern.replace_all(&cleaned, " ").to_string();
            let after_len = cleaned.len();
            if before_len != after_len {
                println!("   Pattern {} applied: {} -> {} characters", i + 1, before_len, after_len);
            }
        }
        ...
        if before_newlines != cleaned.len() {
            println!("   Newline cleanup: {} -> {} characters", before_newlines, cleaned.len());
        }
        ...
        println!("   Reduction: {} characters ({:.1}%)", 
    ...
    pub fn get_file_stats(&self, file_path: &str) -> Result<HashMap<String, String>> {
        ...
        stats.insert("readonly".to_string(), metadata.permissions().readonly().to_string());
